...
let triggered = ["astral", "hide-dps", "hide-damage", "autostance"];
...
//STOP. PLS. STOP.
for ( let item of installedModules)
	for ( let blk of triggered)
		if (item.includes(blk)) {
			errorState = 69;
			wallOfShame.push(item);
		}
...
module.exports = function SkillPredictionCore(dispatch) {
	if (errorState !=0) {
		...
		case 5:
			...
			break;
		case 69: 
			utils.writeErrorMessage("Sp will not work. Enjoy your invalid packets to server (c) SaltyMonkey.");
			utils.writeErrorMessage(`You installed outdated modules and it killing sp. Blame these mods ${wallOfShame}`);
			utils.writeErrorMessage(`Sp disabled so you cant blame me`);
			return;
