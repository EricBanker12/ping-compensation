...
class Cmds {
	constructor(dispatch) {
		...
		//Commands
		command.add("sp", (arg1, arg2) => {
			let option = "";
			let value = "";
			if(arg1 && arg1.length > 0) option = arg1.toLowerCase();
			if(arg2 && arg2.length > 0) value = arg2.toLowerCase();
			switch (option) {
			case "info":
			...
			case "jittercomp":
				if (state.config.jitterCompensation) {
					command.message("[Skill Prediction] jitterCompensation Disabled.");
				} else {
					command.message("[Skill Prediction] jitterCompensation Enabled");
				}
