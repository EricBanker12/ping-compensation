...
class ActivePing {
	constructor(dispatch) {
		...
		let ping = () => {
			...
			waiting = true
			lastSent = Date.now()
			timeout = setTimeout(ping, this.PING_TIMEOUT)
		}

		dispatch.hook('S_SPAWN_ME', 'raw', () => {
			clearTimeout(timeout)
			timeout = setTimeout(ping, this.PING_INTERVAL)
		})
		...
		dispatch.hook('S_RESPONSE_GAMESTAT_PONG', 'raw', () => {
			...
			waiting = false
			timeout = setTimeout(ping, this.PING_INTERVAL - result)
			return false
		})
	}
}
...
class PassivePing {
	constructor(dispatch) {
		this.min = this.max = this.avg = 0;
		this.history = [];
		this.PING_LIMIT_MIN = config.pingSpikesMin;
		this.PING_LIMIT_MAX = config.pingSpikesMax;
		this.PING_HISTORY_MAX = config.pingHistoryMax;
		this.pingLimit = config.pingSpikesLimit;

		const updatePing = ping => {
			if (this.pingLimit && ((ping <= this.PING_LIMIT_MIN) || (ping >= this.PING_LIMIT_MAX))) return;
			this.history.push(ping);
			if (this.history.length > this.PING_HISTORY_MAX) this.history.shift();
