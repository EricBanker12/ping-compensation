...
class Ping {
  constructor(dispatch) {
    ...
    for(let packet of [
      ...
      ['C_CAN_LOCKON_TARGET', dispatch.base.majorPatchVersion >= 74 ? 3 : 2],
    ]) dispatch.hook(packet[0], packet[1], { /*filter: { fake: false, modified: false },*/ order: 1000 }, skillHook);

    dispatch.hook('C_CANCEL_SKILL', dispatch.base.majorPatchVersion >= 74 ? 3 : 2, e => {
        if (e.skill) delete pingStack[e.skill.id ? e.skill.id : skillId(e.skill)];
    });

    const actionHook = e => {
      if(e.gameId && e.gameId - gameId != 0) return;
      if (e.skill) pingEnd(e.skill.id ? e.skill.id : skillId(e.skill));
    };

    for(let packet of [
      ['S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : dispatch.base.majorPatchVersion >= 74 ? 7 : 6],
      ['S_CANNOT_START_SKILL', dispatch.base.majorPatchVersion >= 74 ? 4 : 3],
      ['S_CAN_LOCKON_TARGET', dispatch.base.majorPatchVersion >= 74 ? 3 : 2],
