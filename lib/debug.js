...
function SPDebug(dispatch) {
	...
	function load() {
		...
		for(let options of [
			['C_START_SKILL', dispatch.base.majorPatchVersion < 74 ? 6 : 7,
				_ => [Number(_.unk), Number(_.moving), Number(_.continue), gameId(_.target)]],
			['C_PRESS_SKILL', dispatch.base.majorPatchVersion < 74 ? 3 : 4,
				_ => _.press],
			['C_START_TARGETED_SKILL', dispatch.base.majorPatchVersion < 74 ? 5 : 6,
				_ => `[${_.targets.map(t => `${gameId(t.id)} ${t.unk}`).join(',')}]`],
			['C_START_COMBO_INSTANT_SKILL', dispatch.base.majorPatchVersion < 74 ? 3 : 4,
				_ => `[${_.targets.map(t => `${t.unk1} ${gameId(t.id)} ${t.unk2}`).join(',')}]`],
			['C_START_INSTANCE_SKILL', dispatch.base.majorPatchVersion < 74 ? 4 : 5,
				_ => [_.unk, `[${_.targets.map(t => `${t.unk1} ${gameId(t.id)} ${t.unk2}`).join(',')}]`]],
			['C_START_INSTANCE_SKILL_EX', dispatch.base.majorPatchVersion < 74 ? 4 : 5,
				_ => [_.unk, gameId(_.projectile)]],
			['C_NOTIMELINE_SKILL', dispatch.base.majorPatchVersion < 74 ? 2 : 3],
	...
	function hookFirst(name, ver, cb) {
		hooks.push(dispatch.hook(name, ver, {order: -999}, event => {
	...
	function hookMod(name, ver, cb) {
		hooks.push(dispatch.hook(name, ver, {order: 999, filter: {fake: null, silenced: null}}, event => {
