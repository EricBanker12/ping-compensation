...
class AbnormalityPrediction {
	constructor(dispatch) {
		...
		dispatch.hook("S_RETURN_TO_LOBBY", def.getVersion("S_RETURN_TO_LOBBY", dispatch), () => {
			this.removeAll();
		});
		...
		dispatch.hook("S_ABNORMALITY_BEGIN", def.getVersion("S_ABNORMALITY_BEGIN", dispatch), abnormalityUpdate.bind(null, "S_ABNORMALITY_BEGIN"));
		dispatch.hook("S_ABNORMALITY_REFRESH", def.getVersion("S_ABNORMALITY_REFRESH", dispatch), abnormalityUpdate.bind(null, "S_ABNORMALITY_REFRESH"));

		dispatch.hook("S_ABNORMALITY_END", def.getVersion("S_ABNORMALITY_END", dispatch), event => {
			if (event.target.equals(this.gameId)) {
	...
	add(id, duration, stacks, delay = 0) {
		...
		let type = this.myAbnormals[id] ? "S_ABNORMALITY_REFRESH" : "S_ABNORMALITY_BEGIN",
			version = this.myAbnormals[id] ? def.getVersion("S_ABNORMALITY_REFRESH", this.dispatch) : def.getVersion("S_ABNORMALITY_BEGIN", this.dispatch);
	...
	remove(id, delay = 0) {
		...
		this.myAbnormals[id].removeTimer = setTimeout(() => {
			...
			this.dispatch.toClient("S_ABNORMALITY_END", def.getVersion("S_ABNORMALITY_END", this.dispatch), {
				target: this.gameId,
				id
			});
			this._remove(id);
