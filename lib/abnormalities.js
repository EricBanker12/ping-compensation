...
class AbnormalityPrediction {
	constructor(dispatch) {
		...
		dispatch.hook('S_LOGIN', dispatch.base.majorPatchVersion >= 67 ? 10: 9, event => { this.gameId = event.gameId });

		dispatch.hook('S_RETURN_TO_LOBBY', 1, () => { this.removeAll() });

		dispatch.hook('S_CREATURE_LIFE', 1, event => {
			if(event.target.equals(this.gameId) && !event.alive) this.removeAll()
		});
