...
class SPAbnormals {
	constructor(mod) {
		...
		mod.hook('S_CREATURE_LIFE', 1, event => {
		...
		let abnormalityUpdate = (type, event) => {
			if(event.target === this.player.gameId) {
				if(mod.settings.debug.abnormals) console.log(abnormals[event.id] == true ? '<X' : '<-', type, event.id, event.duration, event.stacks)
		...
		mod.hook('S_ABNORMALITY_END', 1, event => {
			if(event.target === this.player.gameId) {
				if(mod.settings.debug.abnormals) console.log(abnormals[event.id] == true ? '<X' : '<-', 'S_ABNORMALITY_END', event.id)
	...
	add(id, duration, stacks) {
		...
		if(this.mod.settings.debug.abnormals) console.log('<*', type, id, duration, stacks)
	...
	remove(id) {
		...
		if(this.mod.settings.debug.abnormals) console.log('<* S_ABNORMALITY_END', id)
