...
module.exports = function SPCore(mod) {
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(clientAction) {
			...
			else if(clientAction.skill.type === TYPE_REACTION && (clientAction.skill.id !== player.templateId * 100 + 2 || info.dcType !== 'stand')) {
				sendCannotStartSkill(event.skill)
				return false
			}
		}
			...
			switch(passive.type) {
				case 77:
					switch(passive.method) {
						case 2: effectScale += passive.value; break
						case 3: effectScale += Math.max(0, passive.value - 1); break
					}
					break
				case 82:
					switch(passive.method) {
						case 1: stamina = passive.value; break
						case 2: stamina += passive.value; break
						case 3: stamina += passive.value - 1; break
					}
					break
		...
		if(info.glyphs)
			for(let id in info.glyphs)
				if(player.crests.has(Number(id))) {
					...
					if(glyph.speed) passiveSpeed += glyph.speed - 1
	...
	function sendActionStage(opts) {
		...
		if(info.type === 'charging') realSpeed = (realSpeed + opts.chargeSpeed) * info.timeRate
		if(info.fixedSpeed) {
			animSpeed = 1
