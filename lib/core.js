...
module.exports = function SkillPrediction(dispatch) {
    ...
    let config = null,
        ...
        outOfStaminaSystemMessage = null,
        alive = false,
        inCombat = false,
        inventoryHook = null,
        inventory = null,
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (stamina) {
            if (currentStamina < stamina) {
                sendCannotStartSkill(event.skill);
                sendSystemMessage(outOfStaminaSystemMessage)
                return false
            }
    ...
    //-------------
    function OutofStaminaSmtMessageFix() {
        switch (job) {
            case 9: {
                outOfStaminaSystemMessage = 'SMT_BATTLE_SKILL_FAIL_LOW_ARCANE';
                break;
            }
            case 10: {
                outOfStaminaSystemMessage = 'SMT_BATTLE_SKILL_FAIL_LOW_CHAKRA';
                break;
            }
            case 12: {
                outOfStaminaSystemMessage = 'SMT_BATTLE_SKILL_FAIL_LOW_MOON_LIGHT';
                break;
            }
            default: {
                outOfStaminaSystemMessage = 'SMT_BATTLE_SKILL_FAIL_LOW_STAMINA';
                break;
            }
        }
