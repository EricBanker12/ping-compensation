...
module.exports = function SkillPrediction(dispatch) {
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (job == 2 && skillBase == 8) {
            ...
            breakoLimit = false
            setTimeout(() => { breakoLimit = true }, 499);
        }
        ...
        if (type == 'C_PRESS_SKILL' && !event.press && !(canChainVB && (job == 3 && skillBase == 15))) {
            if (currentAction && currentAction.skill == skill) {
                if (info.type == 'hold' || info.type == 'holdInfinite') {
                    ...
                    if (job == 3) //shit optimization for normal classes (TODO: remove it/rewrite or smth)
                        grantCharge(skill, info, abnormality.exists(KYS_BHS) ? (info.length.length || 1) : currentAction.stage);
                    else
    ...
    function sendActionStage(opts) {
        ...
        if(job==3) { //another hardcoded shit for zerk (TODO: rewrite/remove it)
            if (abnormality.exists(KYS_BHS) && info.type === 'charging') {
