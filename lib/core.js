...
module.exports = function SkillPrediction(dispatch) {
    ...
    function enable() {
        ...
        try {
            ...
            addHook('C_NOTIFY_LOCATION_IN_ACTION', 2, notifyLocation.bind(null, 'C_NOTIFY_LOCATION_IN_ACTION', 1));
            addHook('C_NOTIFY_LOCATION_IN_DASH', 2, notifyLocation.bind(null, 'C_NOTIFY_LOCATION_IN_DASH', 1));
            for (let packet of [
    ...
    function sActionStageHandler(event) {
        if (isMe(event.gameId)) {
            if (config.debug) {
                ...
                if (config.debugLoc) strs.push(...[degrees(event.w), '(' + Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z) + ')']);
    ...
    dispatch.hook('S_EACH_SKILL_RESULT', 5, event => {
        ...
        if (isMe(event.target) && ta.enable) {
            if (config.debug) {
                ...
                if (config.debugLoc) strs.push(...[degrees(ta.w), '(' + Math.round(ta.loc.x), Math.round(ta.loc.y), Math.round(ta.loc.z) + ')']);
    ...
    function sendActionEnd(type, distance) {
        ...
        if (config.debug) debug(['<* S_ACTION_END', skillId(currentAction.skill), type || 0, degrees(currentLocation.w), (distance || 0) + 'u'].join(' '));
    ...
    function enableVB() {
        canChainVB = true;
        if (config.debug) debug('[Skill Prediction] Chained VB enabled');
    }
