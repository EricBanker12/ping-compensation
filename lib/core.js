...
module.exports = function PingCompensationCore(dispatch) {
    ...
    function retryHook(packet, code, data, fake) {
        let event = protocol.parse(dispatch.protocolVersion || dispatch.base.protocolVersion, packet[0], packet[1], data = Buffer.from(data))
        if (fake) {
    ...
    //----------
    // S_LOGIN
    dispatch.hook('S_LOGIN', dispatch.majorPatchVersion >= 77 ? 12 : 11, event => {
        gameId = event.gameId
