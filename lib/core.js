...
module.exports = function SkillPrediction(dispatch) {
	...
	function enable() {
		...
		try {
			...
			addHook("S_ACTION_STAGE", 5, sActionStageHandler);
			addHook("S_CREST_MESSAGE", 2, state.hookFake, sCrestMessageHandler);
			addHook("S_START_COOLTIME_SKILL", 1, cooldownFix);
			addHook("S_DECREASE_COOLTIME_SKILL", 1, cooldownFix);
	...
	function notifyLocation(type, version, event) {
		if(currentAction && currentAction.skill != event.skill) { 
	...
	function handleStartSkill(type, event, info, data, send) {
		...
		if (state.config.emulateAttackSpeedFromSelfBuffs) { // Todo: rewrite it on a universal way xd & line 1104
		...
		if (currentAction) {
			if (currentAction.skill & Flags.CC && (currentAction.skill & templateId * 100 + 2 !== 0xffffff || info.type !== "retaliate")) {
				sendCannotStartSkill(event.skill);
				return false;
			}
		...
		if (!info.noRetry && lastEndedTime != 0)
			retry(() => {
				if ((state.config.skillRetryAlways && type != "C_PRESS_SKILL") || info.retryAlways || currentAction && currentAction.skill == skill && !serverConfirmedAction) return toServerLocked(data);
				return false;
			});
	}
	...
	function sActionEndHandler(event) {
		if (isMe(event.gameId)) {
			...
			if (state.config.emulateAttackSpeedFromSelfBuffs && job == 4) { // Todo: rewrite, remove etc
			...
			if (info) {
				...
				if (currentAction && ((event.skill == currentAction.skill && [2, 13, 33, 25, 29, 43].includes(event.type)) || event.type == 9)) {
					updateLocation(event);
					sendActionEnd(event.type);
				}
				return false;
	...
	dispatch.hook("S_EACH_SKILL_RESULT", 6, event => {
		...
		if (isMe(event.target) && ta.enable) {
			if (state.config.debug) {
				...
				if (state.config.debugLoc) strs.push(...[utils.degrees(ta.w), "(" + Math.round(ta.loc.x), Math.round(ta.loc.y), `${Math.round(ta.loc.z)  })`]);
	...
	function sCrestMessageHandler(event) {
		if (state.config.debug) utils.writeDebugMessage(`<- S_CREST_MESSAGE ${event.unk} ${event.type} ${skillId(event.skill, Flags.Skill)}`);
		if (event.type == 6) {
	...
	function sCannotStartSkillHandler(event) {
		if (state.config.debug) utils.writeDebugMessage(`<- S_CANNOT_START_SKILL ${skillId(event.skill, Flags.Player)}`);

		if (skillInfo(event.skill, true)) {
			if (state.config.skillDelayOnFail && state.config.skillRetryCount && currentAction && (!serverAction || currentAction.skill != serverAction.skill) && event.skill == currentAction.skill - 0x4000000)
	...
	function cCanLockonTargetHandler(event) {
		...
		if (info) {
			...
			dispatch.toClient("S_CAN_LOCKON_TARGET", 1, Object.assign({
				ok
			}, event));
		}
	}
	...
	function sendActionStage(opts) {
		...
		dispatch.toClient("S_ACTION_STAGE", 5, currentAction = {
			gameId: myChar(),
			loc: currentLocation.loc,
			w: currentLocation.w,
			templateId,
