...
module.exports = function SkillPrediction(dispatch) {
    ...
    let config = null,
        ...
        defaultConfigFile = path.join(__dirname, '..', 'config', 'default-config.json'),
        canChainVB = false,
        switchVB = false,
        attackSpeed = 0,
        attackSpeedBonus = 0,
    ...
    skills = dispatch.base.majorPatchVersion >= 67 ? require('../config/data/awakening') : require('../config/data/skills');
    ...
    function LoadConfiguration(path = configFile) {
        try {
            ...
            if (config.skillRetryCount > 3) {
                console.log(`[Skill Prediction] skillRetryCount=${config.skillRetryCount} not allowed! skillRetryCount=1.`);
    ...
    command.add('sp', (option, value) => {
        switch (option) {
            case 'info':
                command.message('Unofficial SP. Date:06/04/18');
                    ...
                    case 'generate':
                        if (!currentAction && !serverAction && !mounted && alive && !inCombat && !sending && config.enabled) {
                            if (ping.history.length === config.pingHistoryMax) {
                                ...
                                command.message('[Skill Prediction] You can test changes and use command /8 sp config save for save');
                            } else {
                                command.message('[Skill Prediction] Not enough measurements for ping. Try to repeat command a bit later.');
                            }
                        } else {
                            command.message('[Skill Prediction] Your character busy, wait a bit please');
                        }
                        break;
                    case 'print':
            ...
            case 'off':
                if (!currentAction && !serverAction && !mounted && alive && !inCombat && !sending && config.enabled) {
                    ...
                    command.message('[Skill Prediction] Your character busy or SP already disabled');
                }
                break;
            case 'on':
                if (!currentAction && !serverAction && !mounted && alive && !inCombat && !sending && !config.enabled) {
                    ...
                    command.message('[Skill Prediction] Your character busy or SP already enabled');
                }
                break;
            case 'ping':
                switch (value) {
    ...
    dispatch.hook('S_CREST_APPLY', 2, event => {
        if (config.debugGlyphs) console.log('Glyph', event.id, event.enable);
    ...
    dispatch.hook('S_PARTY_MEMBER_LIST', dispatch.base.majorPatchVersion >= 67 ? 6 : 5, (event) => {
        ...
        for (let member of event.members)
            if (!((dispatch.base.majorPatchVersion >= 67 ? member.gameId : member.cid).equals(gameId)))
    ...
    dispatch.hook('S_MOUNT_VEHICLE', 2, event => {
        if (event.gameId.equals(gameId)) {
            mounted = true;
            if (config.debug) utils.writeDebugMessage('[Skill Prediction] Your character mounted')
        }
    });
    ...
    dispatch.hook('S_UNMOUNT_VEHICLE', 2, event => {
        if (event.gameId.equals(gameId)) {
            mounted = false;
            if (config.debug) utils.writeDebugMessage('[Skill Prediction] Your character unmounted')
        }
    });
    ...
    function startSkill(type, version, code, data) {
        ...
        if (info && info.type === "charging" && Date.now() - lastEndedTime <= (config.skillRetryCount * config.skillRetryMs)) delay += (config.skillRetryCount * config.skillRetryMs);
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (type == 'C_PRESS_SKILL' && event.press && !canChainVB && job == 3 && skillBase == 15 && switchVB) return false;
        ...
        if (currentAction) {
            ...
            if (canChainVB && currentSkillBase == 15 && currentSkillSub == 14) {
                ...
                canChainVB = false;
                if (config.debug) utils.writeDebugMessage('[Skill Prediction] Chained VB disabled');
            }
        ...
        if (info.bodyRolls && config.bodyRollsCheck) {
    ...
    function cCancelSkillHandler(event) {
        ...
        if (currentAction) {
            ...
            if (info && info.blockCancelPacket) {
                if (config.debug) utils.writeDebugMessage('[Skill Prediction] C_CANCEL_SKILL was dropped');
                return false
            }
        }
    }
        ...
        if (isMe(event.gameId)) {
            ...
            if (info) {
                if (info.type == 'dash')
                    ...
                    else if (!lastEndLocation || lastEndLocation.loc.dist3D(event.loc) >= 100)
            ...
            if (!currentAction)
                utils.writeDebugMessage('[Skill Prediction] S_ACTION_END: currentAction is null', skillId(event.skill), event.id);
            else if (event.skill != currentAction.skill)
                utils.writeDebugMessage('[Skill Prediction] S_ACTION_END: skill mismatch', skillId(currentAction.skill), skillId(event.skill), currentAction.id, event.id);
            ...
            case 'charging':
                ...
                if (info.autoRelease !== undefined) {
                    ...
                    if (info.autoRelease === 0) {
                        stageEnd();
                        stageEnd = null
                    } else stageEndTimeout = setTimeout(stageEnd, Math.round(info.lastChargeStage)) // /  glyph.chargeSpeed
                }
    ...
    function enableVB() {
        canChainVB = true;
        if (config.debug) utils.writeDebugMessage('[Skill Prediction] Chained VB enabled');
    }
