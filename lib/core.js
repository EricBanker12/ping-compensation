...
module.exports = function PingCompensationCore(dispatch) {
    ...
    function retryHook(packet, code, data, fake) {
        let event
        if (protocol) event = protocol.parse(dispatch.protocolVersion || dispatch.base.protocolVersion, packet[0], packet[1], data = Buffer.from(data))
        if (!event) event = dispatch.dispatch.fromRaw(packet[0], packet[1], data = Buffer.from(data))
        if (fake) {
