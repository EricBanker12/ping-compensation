...
module.exports = function SkillPrediction(dispatch) {
    ...
    function LoadConfiguration(path = configFile) {
        try {
            ...
            utils.writeErrorMessage(`Smth wrong with config.json... Error: ${err}`);
            return;
        }
    }
    ...
    function SaveConfiguration() {
        fs.writeFile(configFile, JSON.stringify(config, null, "\t"), (err) => {
            if (err)
               utils.writeErrorMessage(`Can't save config! ${err}`)
        })
    ...
    command.add('sp', (option, value) => {
        switch (option) {
            case 'info':
                command.message('Unofficial SP. Date:13/04/18');
    ...
    function cPlayerLocationHandler(event) {
        if (config.debugLoc) utils.writeDebugMessage('Location %d %d (%d %d %d %d) > (%d %d %d)', event.type, event.speed, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), event.w, Math.round(event.dest.x), Math.round(event.dest.y), Math.round(event.dest.z));
        if (currentAction) {
    ...
    function notifyLocation(type, version, event) {
        if (config.debugLoc) utils.writeDebugMessage('-> %s %s %d (%d %d %d %d)', type, skillId(event.skill), event.stage, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), event.w);
        ...
        if (isMe(event.gameId)) {
            ...
            if (info) {
                ...
                if (Math.abs(event.loc.z - currentLocation.loc.z) >= Z_CORRECTION_DIFF) {
                    if (currentAction) {
                        if(config.debug) utils.writeDebugMessage("Z correction queued",event.loc);
                        oopsLocation = event;
                    } else sendInstantMove(event);
                }
