...
module.exports = function SPCore(mod) {
	...
	mod.hook('S_ACTION_STAGE', mod.majorPatchVersion < 75 ? 7 : 8, event => {
		if(isMe(event.gameId)) {
			...
			if(info) {
				...
				else if(currentAction) console.log(`SkillID mismatch ${skillId(currentAction.skill)} ${skillId(event.skill)}`)
	...
	function sendActionStage(opts) {
		...
		mod.toClient('S_ACTION_STAGE', mod.majorPatchVersion < 75 ? 7 : 8, currentAction = {
			gameId: myChar(),
			loc: myPosition.loc,
			w: myPosition.w,
