...
module.exports = function SkillPrediction(dispatch) {
    ...
    command.add('sp', (option, value) => {
        switch (option) {
            case 'info':
                command.message('Unofficial SP. Date:02/02/18');
            ...
            case 'autoconfig':
                if (!currentAction && !serverAction && !mounted && alive && !inCombat && !sending && config.enabled) {
                    if (ping.history.length === config.pingHistoryMax) {
                        ...
                        command.message('[Skill Prediction] You can test changes and use command /8 sp save for save');
                    }
                    else {
            ...
            case 'load':
                LoadConfiguration();
                command.message('[Skill Prediction] Configuration file loaded.');
                break;
            case 'off':
                if (!currentAction && !serverAction && !mounted && alive && !inCombat && !sending && config.enabled) {
                    disable();
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (currentAction) {
            ...
            if (canChainVB && currentSkillBase == 15 && currentSkillSub == 14) {
                ...
                canChainVB = false;
                if (config.debug) console.log('[SP] Chained VB disabled');
            }
        ...
        //ALL MAGIC: (aspd * (1+ sum from (abnormals speed + ghhlyphs))*(1+modifier from nocts) + (sum from (abn+glyphs for chargeSpeed))
    ...
    function enableVB() {
        canChainVB = true;
        if (config.debug) console.log('[SP] Chained VB enabled');
    }
