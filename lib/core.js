...
module.exports = function SPCore(mod) {
	...
	function notifyLocation(type, version, event) {
		...
		if(info) {
			...
			retry('notify', info, () => clientAction && clientAction.skill.equals(event.skill) && mod.send(...arguments))
		}
	}
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(!info.noRetry)
			retry('skill', info, () => clientAction && clientAction.skill.equals(skill) && toServerLocked(data))
	}
	...
	function retry(type, skill, cb) {
		...
		const count = mod.settings.skills[skill.longRetry ? 'longRetryCount' : 'retryCount'],
		...
		function retryIter() {
			...
			retryTimer[type] = setTimeout(() => { if(cb()) retryIter() }, ms)
		}
