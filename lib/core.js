...
module.exports = function PingCompensationCore(dispatch) {
    ...
    const skills = dispatch.base.majorPatchVersion >= 75 ? require('../config/data/skills.75.js') : require('../config/data/skills.js')
    ...
    function skillInfo(id, local = true) {
    ...
    */
    ...
    //----------
    // S_ACTION_STAGE
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : 7, {order: 10, filter: {fake: false}}, event => {
    ...
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : 7, {order: 10, filter: {fake: true}}, event => {
        // if character is your character
        if (event.gameId.equals(gameId)) {
            if (alive && enabled) {
                // bad config idiot-proofing
                if (retryTimer) clearTimeout(retryTimer)
                let group = Math.floor(event.skill.id / 10000)
                // if SP skill still enabled in PC
                if (get(preset, job, "enabled") && get(preset, job, group)) {
                    // disable skill
                    preset[job][group] = false
                }
                // if modded skill
                if (currentAction /*&& !timeouts[currentAction.id]*/) {
    ...
    // sActionStage
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : 7, {filter: {fake: null, modified: null}, order: -100}, event => {
