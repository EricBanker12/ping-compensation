...
module.exports = function SkillPrediction(dispatch) {
        ...
        switch (option) {
            ...
            */
            ...
            case 'on':
                ...
                break;
            case 'ping':
                command.message(`Ping: Min=${ping.min} Avg=${ping.avg} Max=${ping.max} Jitter=${ping.max - ping.min} Samples=${ping.history.length}`);
                break;
            //case 'clearskills':
    ...
    function sendActionStage(opts) {
        ...
        if (info.canVB && info.pendingStartTime) setTimeout(enableVB, (info.pendingStartTime / speed));	// We need to emulate the pending startTime 
        // pendingStartTime / timeRate 
