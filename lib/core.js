...
module.exports = function SkillPrediction(dispatch) {
	...
	function startSkill(type, version, code, data) {
		...
		if (info && !info.canInstantCharge && info.type === "charging" && (Date.now() - lastEndedTime <= (state.config.skillRetryCount * state.config.skillRetryMs))) delay += state.ChargeDelayConst;
	...
	function handleStartSkill(type, event, info, data) {
		...
		if (currentAction) {
			...
			if (currentAction.skill.type === PLAYER_ACTION) {
				...
				if (state.globalNoInterrups[currentAction.skill.id]) { sendCannotStartSkill(event.skill); return false;}
