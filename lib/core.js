		...
		if(type === 'C_PRESS_SKILL' && !event.press) {
			...
			if(releaseChain && clientAction &&
				...
				const prevInfo = skillInfo(clientAction.skill),
					chain = modifyChain(skill, releaseChain.chain),
					nextInfo = skillInfo(chain)

				if(prevInfo && Date.now() - lastStartTime >= (() => {
					switch(nextInfo.skillPendingType) {
						case 'rearCancel': return prevInfo.rearCancelStartTime
						case 'immediateCancel': return prevInfo.pendingStartTime
						case 'superCancel': return 0
