...
module.exports = function PingCompensationCore(dispatch) {
    ...
    function endSkill(event) {
        if (alive && enabled && event) {
            ...
            dispatch.toClient('S_ACTION_END', dispatch.base.majorPatchVersion >= 74 ? 5 : 4, event)
        }
    }
    ...
    function skillHook(event) {
        ...
        let info = skillInfo(event.skill.id, true)
    ...
    function retryHook(packet, code, data) {
        let event = protocol.parse(dispatch.base.protocolVersion, packet[0], packet[1], data = Buffer.from(data)),
            info = skillInfo(event.skill.id, true)
    ...
    //----------
    // C_PRESS_SKILL
    dispatch.hook('C_PRESS_SKILL', dispatch.base.majorPatchVersion >= 74 ? 4 : 3, {order: 10, filter: {fake: null}}, event => {
        updateCoord(event)
        /*
    ...
    dispatch.hook('C_NOTIFY_LOCATION_IN_ACTION', dispatch.base.majorPatchVersion >= 74 ? 4 : 3, {order: 10, filter: {fake: null}}, event => {
        updateCoord(event)
    })
    
    // C_NOTIFY_LOCATION_IN_DASH
    dispatch.hook('C_NOTIFY_LOCATION_IN_DASH', dispatch.base.majorPatchVersion >= 74 ? 4 : 3, {order: 10, filter: {fake: null}}, event => {
        updateCoord(event)
    })
    
    // S_INSTANT_DASH
    dispatch.hook('S_INSTANT_DASH', 3, {order: 10, filter: {fake: null}}, event => {
        if (event.gameId - gameId == 0){
            updateCoord(event)
        }
    })
    ...
    */
    ...
    for(let packet of [
        ...
        //['C_PRESS_SKILL', 1],
        ['C_NOTIMELINE_SKILL', dispatch.base.majorPatchVersion >= 74 ? 3 : 2], //not sure about this one
    ...
    dispatch.hook('S_CANNOT_START_SKILL', dispatch.base.majorPatchVersion >= 74 ? 4 : 3, {order: 10, filter: {fake: null}}, event => {
        if (config.debug) {
    ...
    // C_CANCEL_SKILL
    dispatch.hook('C_CANCEL_SKILL', dispatch.base.majorPatchVersion >= 74 ? 3 : 2, {order: 10, filter: {fake: null}}, event => {
        if (config.debug) {
    ...
    //----------
    // S_ACTION_STAGE
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : dispatch.base.majorPatchVersion >= 74 ? 7 : 6, {order: 10, filter: {fake: false}}, event => {
    ...
    // S_ACTION_END
    dispatch.hook('S_ACTION_END', dispatch.base.majorPatchVersion >= 74 ? 5 : 4, {order: 10, filter: {fake: false}}, event => {
        // if character is your character
        if (event.gameId - gameId == 0) {
            // if modded skill
    ...
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : dispatch.base.majorPatchVersion >= 74 ? 7 : 6, {order: 10, filter: {fake: true}}, event => {
        // if character is your character
        if (event.gameId - gameId == 0) {
            // if modded skill
    ...
    dispatch.hook('S_ACTION_END', dispatch.base.majorPatchVersion >= 74 ? 5 : 4, {order: 10, filter: {fake: true}}, event => {
        // if character is your character
        if (event.gameId - gameId == 0) {
            // if modded skill
            if (alive && enabled && currentAction && currentAction.id == event.id) {
    ...
    dispatch.hook('S_START_COOLTIME_SKILL', dispatch.base.majorPatchVersion >= 74 ? 3 : 2, event => {
    ...
    // sActionStage
    dispatch.hook('S_ACTION_STAGE', dispatch.base.majorPatchVersion >= 75 ? 8 : dispatch.base.majorPatchVersion >= 74 ? 7 : 6, {filter: {fake: null, modified: null}, order: -100}, event => {
        // if character is your character, cancel retry
        if (event.gameId - gameId == 0) clearTimeout(retryTimer)
    })
    ...
    //----------
    // S_EACH_SKILL_RESULT
    dispatch.hook('S_EACH_SKILL_RESULT', dispatch.base.majorPatchVersion >= 74 ? 12 : 10, event => {
