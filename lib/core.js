	...
	function handleStartSkill(type, event, info, data) {
		...
		for(let i = 0; i < 10; i++) { // Limit recursion to prevent infinite loop
			...
			if(chain === undefined && currentAction) {
				...
				if(categoryChains) {
					const category = get(skillsClient, currentAction.templateId, currentAction.skill.id, 'category')

					if(category && category.length)
						for(let id in categoryChains)
							if(category.includes(Number(id)))
								chain = categoryChains[id]
				}
				else if(chains) {
