...
module.exports = function SkillPredictionCore(dispatch) {
	const player = mod(dispatch, './player'),
		ping = mod(dispatch, './ping'),
		abnormality = mod(dispatch, './abnormalities'),
		cooldowns = mod(dispatch, './cooldowns')

	if(settings.DEBUG) mod(dispatch, './debug').toggle() // Lazy implementation for now
	...
	dispatch.hook('S_CREST_APPLY', 2, event => { currentGlyphs[event.id] = event.enable })
	...
	dispatch.hook('S_PARTY_MEMBER_LIST', 7, event => { partyMembers = event.members.map(m => m.gameId) })
	dispatch.hook('S_LEAVE_PARTY', 'raw', () => { partyMembers = null })

	dispatch.hook('S_MOUNT_VEHICLE_EX', 1, event => { if(event.target === player.gameId) vehicleEx = event.vehicle })
	dispatch.hook('S_UNMOUNT_VEHICLE_EX', 1, event => { if(event.target === player.gameId) vehicleEx = null })
	...
	for(let packet of [
		...
		dispatch.hook(...packet, {order: -10, filter: {fake: null}}, notifyLocation.bind(null, ...packet))
	...
	function notifyLocation(type, version, event) {
		...
		if(!currentAction || !currentAction.skill.equals(event.skill)) return false
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(type == 'C_PRESS_SKILL' && !event.press) {
			...
			if(info.overchargeReleaseChain && currentAction && currentAction.overcharge) {
				const prevInfo = skillInfo(currentAction.skill)

				if(prevInfo && Date.now() - lastStartTime >= prevInfo.rearCancelStartTime / currentAction.speed) {
		...
		if(currentAction) {
			...
			if(currentAction.skill.type === TYPE_ACTION) {
				// If checkReset flag is true, prevent re-casting the same skill unless the server says cooldown reset
				if(currentSkill === skill.id && info.checkReset && !currentAction.reset) {
					sendCannotStartSkill(event.skill)
					return false
				}
			...
			else if(currentAction.skill.type === TYPE_REACTION && (currentAction.skill.id !== player.templateId * 100 + 2 || info.type !== 'retaliate')) {
				sendCannotStartSkill(event.skill)
				return false
			}
		}
		...
		if(!info.noRetry)
			retry(() => {
				if((settings.SKILL_RETRY_ALWAYS && type !== 'C_PRESS_SKILL') || currentAction && currentAction.skill.equals(skill)) return toServerLocked(data)
				return false
			})
	}
	...
	dispatch.hook('S_CREST_MESSAGE', 2, event => {
		if(event.type === 6 && currentAction && currentAction.skill.type === TYPE_ACTION && currentAction.skill.id === event.skill)
			currentAction.reset = true
	})

	dispatch.hook('S_ACTION_STAGE', dispatch.majorPatchVersion < 75 ? 7 : 8, event => {
		if(isMe(event.gameId)) {
			if(settings.DEBUG) {
			...
			if(info) {
				...
				if(info.forceClip && event.movement.length) {
					...
					if(!currentAction || !currentAction.skill.equals(event.skill)) sendInstantMove(oopsPosition)
				}
	...
	dispatch.hook('S_GRANT_SKILL', 3, event => skillInfo(modifyChain(event.skill, 0)) ? false : undefined)
	...
	dispatch.hook('S_ACTION_END', 5, event => {
		if(isMe(event.gameId)) {
			...
			if(info) {
				...
				if(currentAction && event.skill.equals(currentAction.skill) && [2, 13, 25, 29, 43].includes(event.type)) {
					updatePosition(event)
					sendActionEnd(event.type)
				}
			...
			if(!currentAction)
				console.log(`[SkillPrediction] (S_ACTION_END) currentAction is null: ${skillId(event.skill)}`)
			else if(!event.skill.equals(currentAction.skill))
				console.log(`[SkillPrediction] (S_ACTION_END) skill mismatch: ${skillId(event.skill)} (expected ${skillId(currentAction.skill)})`)

			currentAction = null
		}
	})

	dispatch.hook('S_EACH_SKILL_RESULT', 9, event => {
		...
		if(isMe(event.target) && reaction.enable) {
			if(settings.DEBUG) {
				...
				if(settings.DEBUG_LOC) strs.push(...[degrees(reaction.w), '(' + Math.round(reaction.loc.x), Math.round(reaction.loc.y), Math.round(reaction.loc.z) + ')'])
	...
	dispatch.hook('S_DEFEND_SUCCESS', 3, event => {
		if(isMe(event.gameId))
			...
			else if(currentAction && currentAction.skill.equals(serverAction.skill)) currentAction.defendSuccess = true
	...
	dispatch.hook('S_CANNOT_START_SKILL', 4, event => {
		if(skillInfo(event.skill.id)) {
			if(settings.SKILL_DELAY_ON_FAIL && settings.SKILL_RETRY_COUNT && currentAction && (!serverAction || !currentAction.skill.equals(serverAction.skill)) && event.skill.id === currentAction.skill.id)
	...
	dispatch.hook('C_CAN_LOCKON_TARGET', 3, event => {
		const info = skillInfo(event.skill)
		if(info) {
			let success = true
			...
			dispatch.toClient('S_CAN_LOCKON_TARGET', 3, Object.assign({success}, event))
		}
	})

	dispatch.hook('S_CAN_LOCKON_TARGET', 3, event => skillInfo(event.skill) ? false : undefined)
	...
	function sendActionStage(opts) {
		if(settings.DEBUG) debug(`<* S_ACTION_STAGE ${skillId(opts.skill)} ${opts.stage}`)
		...
		dispatch.toClient('S_ACTION_STAGE', dispatch.majorPatchVersion < 75 ? 7 : 8, currentAction = {
			gameId: myChar(),
			loc: myPosition.loc,
			w: myPosition.w,
			...
			overcharge: opts.overcharge || !!currentAction && currentAction.overcharge
		})
		...
		if((info.type === 'charging' || info.type === 'holdInfinite') && opts.stage === ((info.length && (info.length.length || 1)) || 0)) {
			...
			effectsTimeouts.push(setTimeout(() => { currentAction.overcharge = true }, Math.round(info.overcharge / speed)))
	...
	function grantCharge(skill, info, stage) {
		...
		dispatch.toClient('S_GRANT_SKILL', 3, {skill: modifyChain(skill.clone(), levels ? levels[stage] : 10 + stage)})
	}
	...
	function sendActionEnd(type, distance) {
		...
		if(currentAction.id === actionNumber) { // Emulated skill
			const info = skillInfo(currentAction.skill)
			if(info) {
				...
				if(info.cooldownEnd && !cooldowns.check(currentAction.skill))
					cooldowns.set(currentAction.skill, info.cooldownEnd)
	...
	function sendCannotStartSkill(skill) { dispatch.toClient('S_CANNOT_START_SKILL', 4, { skill: new SkillID({id: skill.id}) }) }
	function sendSystemMessage(type, vars) { dispatch.toClient('S_SYSTEM_MESSAGE', 1, { message: dispatch.buildSystemMessage(type, vars) })  }
	...
	function isMe(id) {
		return player.gameId === id || vehicleEx && vehicleEx === id
	}
