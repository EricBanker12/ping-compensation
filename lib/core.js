...
module.exports = function PingCompensationCore(dispatch) {
    ...
    */
    ...
    //----------
    // S_ACTION_STAGE
    dispatch.hook('S_ACTION_STAGE', 5, {order: 10, filter: {fake: false}}, event => {
    ...
    dispatch.hook('S_ACTION_STAGE', 5, {order: 10, filter: {fake: true}}, event => {
        // if character is your character
        if (event.gameId.equals(gameId)) {
            // if modded skill
    ...
    // sActionStage
    dispatch.hook('S_ACTION_STAGE', 5, {filter: {fake: null, modified: null}, order: -100}, event => {
