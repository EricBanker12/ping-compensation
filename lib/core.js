...
module.exports = function SkillPrediction(dispatch) {
    ...
    command.add('sp', (option, value) => {
        switch (option) {
            case 'info':
                command.message('Unofficial SP. Date:21/04/18');
    ...
    function sDefendSuccessHandler(event) {
        if (isMe(event.cid))
            try{
                ...
                utils.writeDebugMessage("Current action state: " +currentAction);
                utils.writeDebugMessage("Server action state: " + serverAction);
            }
    }
