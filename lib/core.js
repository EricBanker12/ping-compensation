...
module.exports = function SPCore(mod) {
	...
	function notifyLocation(type, version, event) {
		if(mod.settings.debug.loc) console.log('-> %s %s %d (%d %d %d %s)', type, skillId(event.skill), event.stage, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), degrees(event.w))
		...
		// The server ignores packets with an incorrect skill, so correct and resend later if necessary
		if(info && info.hasChains)
			...
			else pendingNotifyLocation = [type, version, event]
	}
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(clientAction) {
			...
			if(clientAction.skill.type === TYPE_ACTION) {
				...
				reaction: {
					if(
						clientAction.air && info.useSkillWhileAirReaction ||
						clientAction.skill.id === player.templateId*100 + 2 && info.dcType === 'stand'
					)
						break reaction
		...
		if(!info.noRetry)
			retry(
				info.longRetry ? mod.settings.skills.longRetryCount : mod.settings.skills.retryCount,
				info.longRetry ? mod.settings.skills.longRetryMs : mod.settings.skills.retryMs,
				() => {
					if((mod.settings.skills.retryAlways && type !== 'C_PRESS_SKILL') || clientAction && clientAction.skill.equals(skill))
	...
	function sendActionStage(opts) {
		...
		if(info.fixedSpeed) {
			animSpeed = 1
			if(!['movingskill', 'shootingmovingskill'].includes(info.dcType)) realSpeed = 1
		}
		...
		if((info.dcType === 'movingcharge' || info.type === 'holdInfinite') && opts.stage === ((info.length && (info.length.length || 1)) || 0)) {
		...
		if(info.dcType === 'movingcharge' || info.type === 'holdInfinite') {
			opts.stage += 1
			stageEnd = sendActionStage.bind(null, opts)
			stageEndTime = Date.now() + length
			stageEndTimeout = setTimeout(stageEnd, length)
		...
		stageEnd = sendActionEnd.bind(null, info.type === 'dash' ? 39 : 0, opts.distance * opts.distanceMult, moveDir)
		stageEndTime = Date.now() + length
		stageEndTimeout = setTimeout(stageEnd, length)
	}
	...
	function retry(count, ms, cb, iter = 1) {
		if(iter > count) return

		setTimeout(() => {
			if(cb()) retry(count, ms, cb, iter + 1)
		}, ms)
	}
