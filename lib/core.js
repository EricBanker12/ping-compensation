const {protocol} = require("tera-data-parser"),
	...
	silence = utils.getMapFromArray(require("../config/data/basicCC")),
	evasivecc = utils.getMapFromArray(require("../config/data/evasiveCC")),
	extcc = utils.getMapFromArray(require("../config/data/extendedCC"));
...
module.exports = function SkillPrediction(dispatch) {
    ...
    const FAKE_HOOK = {order: -10, filter: {fake: null}};
	...
	function enable() {
		...
		try {
			...
			for (let packet of [
                ...
                addHook(packet[0], "raw", FAKE_HOOK, startSkill.bind(null, ...packet));
			...
			addHook("S_CREST_MESSAGE", 2, FAKE_HOOK, sCrestMessageHandler);
	...
	command.add("sp", (option, value) => {
		switch (option) {
		case "info":
			command.message("Unofficial SP. Date:20/05/18");
		...
		if (isMe(event.gameId)) {
			...
			if (info) {
				...
				// If the skill ends early then there should be no significant error
					...
					else if (!lastEndLocation || lastEndLocation.loc.dist2D(event.loc) >= XY_CORRECTION_DIFF)
	...
	function sendActionStage(opts) {
		...
		if (config.debug) utils.writeDebugMessage(`<* S_ACTION_STAGE ${skillId(opts.skill)} ${opts.stage} ${info.type == "charging" ? 1 : opts.speed}`);
