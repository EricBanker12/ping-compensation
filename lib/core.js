...
module.exports = function SkillPrediction(dispatch) {
    const ping = Ping(dispatch),
        ...
        TELEPORT_UNBLOCK_DELAY = 30; //client retarded: applying cd AFTER real actionEnd with DELAY! BUT RECEIVED IT BEFORE with ActionStage ((c) SaltyMonkey)
    ...
    function cPlayerLocationHandler(event) {
        if (config.debugLoc) utils.writeDebugMessage(`Location type: ${event.type} speed: ${event.speed} (${event.loc}) > (${event.dest})`);
        if (currentAction) {
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        /*
        if (info.type == "teleport" && blockedSkills[skillBase]) {
            if (config.debug) utils.writeDebugMessage(`Teleport doublecast protection: cant cast ${skillBase}, return`);
            sendCannotStartSkill(event.skill);
            return false;
        }
        */
        ...
        if (info.type == "teleport" && !blockedSkills[skillBase]) {
            ...
            if (config.debug) utils.writeDebugMessage(`Teleport doublecast protection: skill blocked ${skillBase}`);
        }
        */
    ...
    function sActionEndHandler(event) {
        if (isMe(event.gameId)) {
            ...
            if (info) {
                ...
                /*
                const skillBase = Math.floor((event.skill - 0x4000000) / 10000);
                if (blockedSkills[skillBase]) {
                    setTimeout(() => {
                        delete blockedSkills[skillBase];
                    }, TELEPORT_UNBLOCK_DELAY);
                }
                */
    ...
    function sCrestMessageHandler(event) {
        ...
        if (event.type === 6 && currentAction && event.skill === currentAction.skill - Flags.Player)
    ...
    function startAction(opts) {
        ...
        if (info.triggerAbnormalOnCondition)
            for (let id in info.triggerAbnormalOnCondition) {
                if (!abnormality.exists(id)) {
                    let abProperties = info.triggerAbnormalOnCondition[id];
    ...
    function getServerTimeout() {
        return (ping.min * 2) + (ping.max > 300 ? 130 : ping.max / 2) + (config.skillRetryCount * config.skillRetryMs) + config.serverTimeout
    }
