...
module.exports = function SPCore(mod) {
	...
	if(mod.settings.debug.skills) subMod(mod, './debug').toggle() // Lazy implementation for now
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(info.onlyDefenceSuccess)
			if(currentAction && currentAction.defendSuccess) endReason = 3
			else {
				sendCannotStartSkill(event.skill)
		...
		if(!info.noRetry)
			retry(() => {
				if((mod.settings.skills.retryAlways && type !== 'C_PRESS_SKILL') || currentAction && currentAction.skill.equals(skill)) return toServerLocked(data)
				return false
			})
	}
	...
	mod.hook('S_CANNOT_START_SKILL', 4, event => {
		if(skillInfo(event.skill.id)) {
			if(mod.settings.skills.delayOnFail && mod.settings.skills.retryCount && currentAction && (!serverAction || !currentAction.skill.equals(serverAction.skill)) && event.skill.id === currentAction.skill.id)
	...
	function sendActionStage(opts) {
		if(mod.settings.debug.skills) debug(`<* S_ACTION_STAGE ${skillId(opts.skill)} ${opts.stage}`)
		...
		const animSpeed = info.fixedAnimSpeed || info.type === 'charging' ? 1 : opts.speed
		mod.send('S_ACTION_STAGE', mod.majorPatchVersion < 75 ? 7 : 8, currentAction = {
			gameId: myChar(),
			loc: myPosition.loc,
			w: myPosition.w,
	...
	function sendActionEnd(type, distance) {
		...
		if(mod.settings.debug.skills) debug(['<* S_ACTION_END', skillId(currentAction.skill), type || 0, degrees(myPosition.w), (distance || 0) + 'u'].join(' '))
	...
	function skillInfo(skill) {
		...
		if(skill.type === 1) console.log(`[SkillPrediction] Unemulated skill: ${skillId(skill)}`)
